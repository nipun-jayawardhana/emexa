      ...
      try {
        ...
        if (adminViewingAs === 'teacher' && viewingUserId && adminToken) {
          ...
          try {
            ...
            alert('Failed to load teacher profile. Returning to user management.');
...
const handleAvatarChange = async (e) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "fullName" && value.trim() && !isAdminViewing) {
      ...
      window.dispatchEvent(new CustomEvent('userNameChanged', { detail: value }));
    }
  };
    ...
    try {
      ...
      if (isAdminViewing && viewingUserData?._id) {
        ...
        if (response.data?.success || response.data?.data) {
          ...
          window.dispatchEvent(new CustomEvent('userNameChanged', { detail: updatedName }));
  ...
  const handleSubmitChangePassword = async ({ currentPassword, newPassword, confirmPassword }) => {
    ...
    try {
      ...
      const response = await axios.put('/api/teacher/change-password',
...
const handleSavePrivacy = async () => {
  ...
  const handleExportData = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/teacher/export-data', {
        headers: { Authorization: `Bearer ${token}` }
      });
          ...
          <div class="section">
            ...
            <div class="settings-grid">
              <div class="setting-item">
                ...
                <div class="setting-value">${notificationSettings.emailNotifications ? '✓ Enabled' : '✗ Disabled'}</div>
              </div>
              <div class="setting-item">
                <div class="setting-label">SMS Notifications</div>
                <div class="setting-value">${notificationSettings.smsNotifications ? '✓ Enabled' : '✗ Disabled'}</div>
              </div>
              <div class="setting-item">
                <div class="setting-label">In-App Notifications</div>
                <div class="setting-value">${notificationSettings.inAppNotifications ? '✓ Enabled' : '✗ Disabled'}</div>
              </div>
            </div>
          </div>
          ...
          <div class="section">
            ...
            <div class="settings-grid">
              <div class="setting-item">
                ...
                <div class="setting-value">${privacySettings.emotionDataConsent ? '✓ Granted' : '✗ Not Granted'}</div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-yellow-50 border-b border-yellow-200 px-6 py-3 flex items-center justify-between mb-6 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-yellow-800">
              Admin Preview Mode: Viewing {viewingUserData?.name || 'Teacher'}'s Profile
            </span>
          </div>
          <button
            onClick={() => {
        ...
        <div className="bg-white rounded-b-lg shadow-sm p-8">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between py-4 border-b border-gray-100">
                  ...
                  <label className="relative inline-flex items-center cursor-pointer ml-4">
                    ...
                    />
                    <div className={`w-14 h-7 rounded-full peer-focus:ring-4 peer-focus:ring-teal-300 relative transition-all duration-300 ${
                      notificationSettings.inAppNotifications ? 'bg-teal-600' : 'bg-gray-300'
                    }`}>
                      <div className={`absolute top-1 bg-white w-5 h-5 rounded-full shadow-md transition-all duration-300 ${
          ...
          {activeTab === "activity" && (
            <div className="space-y-6">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-400 mt-2">Your teaching activities will appear here</p>
              </div>
            </div>
          )}
          ...
          {activeTab === "privacy" && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-semibold text-gray-900 mb-2">Emotion Data Consent</h3>
                <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <p className="text-sm text-gray-700 mb-3">
                        Allow the application to collect and analyze emotional data during quiz sessions.
                      </p>
                    ...
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 ml-4">
                      ...
                      />
                      <div className={`w-14 h-7 rounded-full peer-focus:ring-4 peer-focus:ring-teal-300 relative transition-all duration-300 ${
                        privacySettings.emotionDataConsent ? 'bg-teal-600' : 'bg-gray-300'
                      }`}>
                        <div className={`absolute top-1 bg-white w-5 h-5 rounded-full shadow-md transition-all duration-300 ${
  ...
  const isActuallyAdminViewing = isAdminViewing && adminToken && localStorage.getItem('adminViewingUserId');
