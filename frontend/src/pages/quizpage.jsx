...
const QuizPage = () => {
  ...
  const [sessionId] = useState(`quiz-${Date.now()}`);
  const [generatingHint, setGeneratingHint] = useState(false);

  // NEW: Emotion tracking states
  ...
  // NEW: Socket.IO connection for emotion tracking
  useEffect(() => {
    console.log('🎮 Quiz: Initializing emotion tracking...');
    ...
    socket.on('connect', () => {
      console.log('✅ Quiz: Emotion socket connected:', socket.id);
    });

    socket.on('emotion-result', (data) => {
      console.log('😊 Quiz: Emotion detected:', data);
      
      if (mountedRef.current) {
        setCurrentEmotion(data);
        setEmotionHistory(prev => [...prev, { ...data, timestamp: Date.now() }]);
      }
    });

    socket.on('emotion-error', (error) => {
      console.error('❌ Quiz: Emotion error:', error);
    });

    socket.on('disconnect', (reason) => {
      console.log('🔌 Quiz: Socket disconnected:', reason);
    });

    socketRef.current = socket;

    return () => {
      console.log('🧹 Quiz: Cleaning up socket');
  ...
  // NEW: Check if camera is running
  useEffect(() => {
    const checkCamera = () => {
      const cameraActive = camera.isActive();
      console.log('📹 Quiz: Camera active?', cameraActive);
      setEmotionTracking(cameraActive);
      
      if (cameraActive) {
        console.log('✅ Quiz: Emotion tracking enabled');
      } else {
        console.log('ℹ️ Quiz: Emotion tracking disabled (camera not active)');
      }
    };
  ...
  // NEW: Cleanup camera when quiz ends
  useEffect(() => {
    return () => {
      console.log('🧹 Quiz ended: Stopping camera');
      if (camera.isActive()) {
        camera.stop();
      }
    };
    ...
    try {
      ...
      if (quizId) {
        try {
          ...
          if (teacherQuiz && teacherQuiz.questions && teacherQuiz.questions.length > 0) {
            const formattedQuestions = teacherQuiz.questions.map((q, index) => ({
              id: index + 1,
              text: q.questionText,
              type: q.type,
              options: q.type === "mcq" ? q.options.map((opt) => opt.text) : [],
              correctAnswer: q.type === "mcq" ? q.options.findIndex((opt) => opt.isCorrect) : null,
              hints: q.hints || ["", "", "", ""],
  ...
  const handleRevealHint = async (index) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/quiz/hint/request`, {
      ...
      if (!updatedQuestions[currentQuestion].hints) 
        updatedQuestions[currentQuestion].hints = ["", "", "", ""];
      updatedQuestions[currentQuestion].hints[index] = response.data.data.hint;
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(`${API_URL}/api/quiz/submit-ai`, {
        userId: localStorage.getItem('userId'),
        quizId, 
        sessionId,
      ...
      sessionStorage.setItem('aiFeedback', response.data.data.feedback);
      sessionStorage.setItem('emotionalSummary', response.data.data.emotionalSummary || '');
      sessionStorage.setItem('hintsUsed', response.data.data.hintsUsed);
      sessionStorage.setItem('finalScore', response.data.data.finalScore);
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .header { background: #0f766e; color: white; padding: 20px; margin: -40px -40px 30px -40px; border-radius: 8px 8px 0 0; }
          .header h1 { margin: 0; font-size: 24px; }
          .success { text-align: center; margin-bottom: 30px; }
          .success h2 { color: #047857; margin-bottom: 10px; }
          .summary { background: #f0fdfa; padding: 20px; border-radius: 8px; margin-bottom: 30px; }
          ...
          .summary-item value { display: block; font-weight: bold; color: #1f2937; }
          .score-box { background: #fef2f2; border-left: 4px solid #ef4444; padding: 20px; margin-bottom: 30px; }
          ...
          .score-percentage { font-size: 36px; font-weight: bold; color: #ef4444; }
          .question-box { border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin-bottom: 20px; page-break-inside: avoid; }
          .question-header { display: flex; justify-content: space-between; margin-bottom: 15px; }
          .question-title { font-weight: bold; color: #1f2937; flex: 1; }
          .answer-box { padding: 12px; border-radius: 8px; margin-bottom: 10px; }
          .correct-answer { background: #f0fdf4; }
          .wrong-answer { background: #fef2f2; }
          .explanation { background: #eff6ff; }
          .answer-label { font-size: 12px; color: #6b7280; margin-bottom: 5px; }
          .answer-text { font-weight: 500; margin: 0; }
      ...
      <body>
        <div class="header"><h1>Quiz Results: ${quizData.title}</h1></div>
        <div class="success">
        ...
        ${quizData.questions.map((question, index) => {
          ...
          const isCorrect = question.type === "mcq" ? userAnswer === question.correctAnswer : false;
          return `
            <div class="question-box">
              <div class="question-header">
                <div class="question-title">Question ${question.id}: ${question.text}</div>
                ${question.type === "mcq" ? `<span class="status-icon">${isCorrect ? "✓" : "✗"}</span>` : ""}
              </div>
              ${userAnswer !== undefined && userAnswer !== "" ? `
                <div class="answer-box ${question.type === "mcq" ? (isCorrect ? "correct-answer" : "wrong-answer") : "explanation"}">
                  <div class="answer-label">Your Answer:</div>
                  <p class="answer-text ${question.type === "mcq" ? (isCorrect ? "correct-text" : "wrong-text") : "explanation-text"}">
              ...
              <div class="answer-box explanation">
                ...
                <p class="answer-text explanation-text">${question.hints && question.hints[3] ? question.hints[3] : "No explanation available"}</p>
              </div>
            </div>
          `;
        }).join("")}
  ...
  if (quizSubmitted) {
    const score = calculateScore();
    const percentage = Math.round((score / quizData.questions.length) * 100);
    const aiFeedback = sessionStorage.getItem('aiFeedback');
    const emotionalSummary = sessionStorage.getItem('emotionalSummary');
    const hintsUsed = sessionStorage.getItem('hintsUsed') || hintsUsedCount;
    const finalScore = sessionStorage.getItem('finalScore') || score;
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full p-8">
          ...
          {aiFeedback && (
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl p-6 mb-8 border-2 border-purple-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-bold text-gray-800">AI Personalized Feedback</h3>
              </div>
              <p className="text-gray-700 leading-relaxed mb-4">{aiFeedback}</p>
              {emotionalSummary && (
                <div className="bg-white rounded-lg p-4 border border-purple-200">
                  <p className="text-sm font-medium text-purple-800">
                    Emotional Pattern: {emotionalSummary}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-red-50 border-l-4 border-red-400 p-6 mb-8">
            ...
            <div className="space-y-2">
              <p className="text-gray-700">
                <span className="font-semibold">Raw Score:</span> {score} / {quizData.questions.length}
              </p>
              <p className="text-gray-700">
                <span className="font-semibold">Hints Used:</span> {hintsUsed}
                {hintsUsed > 0 && <span className="text-red-600"> (-{hintsUsed} marks)</span>}
              </p>
              <p className="text-gray-900 font-bold text-lg">
                Final Score: {finalScore} / {quizData.questions.length}
              </p>
            </div>
          </div>

          {/* Question review section - continues in Part 6 */}
          {/* Continuing from Part 5 - Question review in results */}
          {quizData.questions.map((question, index) => {
            const userAnswer = answers[index];
            const isCorrect = question.type === "mcq" ? userAnswer === question.correctAnswer : false;
                ...
                <div className="space-y-3">
                  ...
                  {question.type === "short" && question.shortAnswer && (
                    <div className="p-3 rounded-lg bg-teal-50">
                      ...
                      <p className="font-medium text-teal-800">{question.shortAnswer}</p>
                    </div>
                  )}
                  ...
                  <div className="p-3 rounded-lg bg-blue-50">
                    ...
                    <p className="text-blue-800">
                      {question.hints && question.hints[3] ? question.hints[3] : "No explanation available"}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-center gap-4 mt-8">
            ...
            >
              Download as PDF
              <img src={DownloadIcon} alt="download icon" className="w-5 h-5 object-contain" />
            </button>
            <button
  ...
  // Main Quiz UI (when not submitted)
  const question = quizData.questions[currentQuestion];

  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-50 flex relative">
      {/* NEW: Emotion indicator (top-right corner) */}
      {emotionTracking && currentEmotion && (
        <div className="fixed top-4 right-4 bg-white shadow-lg rounded-lg p-4 z-50 animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-medium text-gray-500">Emotion Detected</div>
      ...
      {/* NEW: Emotion tracking status indicator */}
      <div className="fixed bottom-4 right-4 z-40">
        {emotionTracking ? (
          <div className="bg-green-500 text-white px-3 py-2 rounded-full shadow-lg flex items-center gap-2 text-sm">
            <Video className="w-4 h-4" />
            <span>Emotion Tracking Active</span>
          </div>
        ) : (
          <div className="bg-gray-400 text-white px-3 py-2 rounded-full shadow-lg flex items-center gap-2 text-sm">
      ...
      <div className="w-64 bg-green-100 p-6 flex flex-col">
        ...
        <div className="mt-auto">
          <div className="flex items-center justify-center gap-2 mb-2">
            <img src={headerLogo} alt="EMEXA Logo" className="w-50 h-50 object-contain" />
          </div>
        </div>
      </div>

      {/* Main content area - continues in Part 8 */}
      {/* Main Content Area */}
      <div className="flex-1 p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-md p-8 relative">
            ...
            {/* Answer Options - MCQ or Short Answer */}
            {question.type === "mcq" ? (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {!revealedHints.includes(index) && (
                          ...
                          >
                            {generatingHint ? "Generating..." : "Reveal"}
                          </button>
                        )}
                      </div>
                      {revealedHints.includes(index) && (
