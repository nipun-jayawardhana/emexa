...
const TeacherActivityTab = () => {
  ...
  const [showQuizDetailModal, setShowQuizDetailModal] = useState(false); // ✅ NEW
  const [quizDetails, setQuizDetails] = useState(null); // ✅ NEW
  ...
  // ✅ NEW: Fetch full quiz details when card is clicked
  const handleQuizCardClick = async (quiz) => {
    try {
      ...
      console.log('✅ Quiz details loaded:', response.data);
      setQuizDetails(response.data.data || response.data);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              {/* Quiz Info Section */}
              <div className="mb-6">
                <h3 className="text-lg font-bold text-gray-900 mb-3">Quiz Information</h3>
                <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                  <div>
                    <p className="text-sm text-gray-600">Created Date</p>
                    <p className="font-medium text-gray-900">{formatDate(quizDetails.createdAt)}</p>
                  </div>
                  {quizDetails.scheduleDate && (
                    <div>
                      <p className="text-sm text-gray-600">Scheduled Date</p>
                      <p className="font-medium text-gray-900">{formatDate(quizDetails.scheduleDate)}</p>
                    </div>
                  )}
                  {quizDetails.dueDate && (
                    <div>
                      <p className="text-sm text-gray-600">Due Date</p>
                      <p className="font-medium text-gray-900">{formatDate(quizDetails.dueDate)}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-sm text-gray-600">Total Questions</p>
                    <p className="font-medium text-gray-900">{quizDetails.questions?.length || 0}</p>
                  </div>
                </div>
              </div>
                    ...
                    quizDetails.questions.map((question, index) => (
                      <div key={index} className="bg-white border border-gray-200 rounded-lg p-5">
                        <div className="flex items-start gap-3 mb-3">
                          <span className="flex-shrink-0 w-8 h-8 bg-teal-100 text-teal-700 rounded-full flex items-center justify-center font-bold text-sm">
                            {index + 1}
                          </span>
                          <div className="flex-1">
                            <p className="font-medium text-gray-900 mb-3">{question.question || question.text}</p>
