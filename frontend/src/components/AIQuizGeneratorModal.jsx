...
import { X, Sparkles, RefreshCw, Save, Eye, AlertCircle, BookOpen, Brain, Clock, Target } from 'lucide-react';
...
const AIQuizGeneratorModal = ({ isOpen, onClose, onQuizGenerated }) => {
  ...
  const fetchSuggestions = async () => {
    try {
      const response = await axios.get('/api/ai-quiz/suggestions', {
        params: {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      const response = await axios.post('/api/ai-quiz/generate', formData);
      
      if (response.data.success) {
        setGeneratedQuiz(response.data.data.quiz);
      }
    } catch (err) {
  ...
  const handleRegenerateQuestion = async (questionNumber) => {
    ...
    try {
      const response = await axios.post(
        `/api/ai-quiz/${generatedQuiz.quizId}/regenerate`,
        {
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      const response = await axios.put(
        `/api/ai-quiz/${generatedQuiz.quizId}`,
        {
          ...generatedQuiz,
          isPublished: false
        }
